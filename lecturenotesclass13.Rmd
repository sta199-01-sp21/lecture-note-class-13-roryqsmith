...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-roryqsmith/manhattan.csv")
```
...
I got 21.5, and not everyone in the class got the same answer. In future iterations of my own generation of sample mean, I got a slightly different answer.
...
**Interpretation**: we are 95% confident that the mean price of one-bedroom apartments in Manhattan is between $2302 and $2974.
...
We are 95% confident that the median price of 1-bedroom apartments in Manhattan is between $2162 and $2875.
...
```{r ci_mice}
ci_mice_died <- get_ci(boot_prop, level = .9)
ci_mice_died
```

What does this mean?

**Interpretation**: we are 95% confident that the true proportion of mice that died while not on the treatment is between 69% and 79.7%.
...
```{r practice_2_ci}
ci_mice_live <- get_ci(boot_prop_live, level = 0.95)
ci_mice_live
```
...
**Interpretation**: I am 90% confident that the actual proportion of mice that lived while on the treatment is between 33.7% and 46.0%.
